...
void open_input_file(const char *file_name, AVFormatContext **in_fmt_ctx, AVCodecContext **dec_ctx)
{
    ...
    AVStream *stream;
    rc = avformat_open_input(in_fmt_ctx, file_name, NULL, NULL);
    success = (rc == 0);
    ...
    if (success)
    {
        ...
        if (!success)
        {
            fprintf(stderr, "ERROR Could not find stream info for %s\n", file_name);
        }
    }

    if (success)
    {
        rc = av_find_best_stream(*in_fmt_ctx, media_type, -1, -1, NULL, 0);
        success = (rc >= 0);
        if (!success)
        {
            fprintf(stderr, "ERROR Could not find %s stream in input file '%s'\n",
                av_get_media_type_string(media_type), file_name);
        }
        else
        {
            stream_index = rc;
    ...
    if (success)
    {
        *dec_ctx = avcodec_alloc_context3(dec);
        ...
        if (!success)
        {
            fprintf(stderr, "ERROR Failed to alloc %s codec context\n",
                    av_get_media_type_string(media_type));
        }
    }
    ...
    if (success)
    {
        rc = avcodec_parameters_to_context(*dec_ctx, stream->codecpar);
        success = (rc == 0);
        if (!success)
        {
            fprintf(stderr, "ERROR copy %s codec pars to decoder ctx\n",
                    av_get_media_type_string(media_type));
        }
    }
